...
def build_omission_label(args):
    ...
    def process_function(ex):
        ...
        # Ensure data is int64 and no nulls exist
        def ensure_int64(data):
            if isinstance(data, list):
                return [ensure_int64(x) for x in data]
            return int(data) if data is not None else -1

        pred_labels = ensure_int64(pred_labels)
        omissions = ensure_int64(omissions)
        omission_words = ensure_int64(omission_words)

        return {
            'oracle_labels': gold_labels if gold_labels else [-1],
...
def group_omission_data(args, models=['bart_base', 't5_small']): # dialsumm은 baseline 빼고 함 나머지는 baseline 넣어줘야 함
