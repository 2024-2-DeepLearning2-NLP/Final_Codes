...
def build_predicted_datasets(args, accelerator, tokenizer, predictions, references, split):
    ...
    def pair_func(ex):
        ...
        sents = dialogue.replace('\r\n', '\n').split('\n') # dialog를 sentence 단위로 자름. 
        new_omission_labels = [[] for _ in range(len(preds))] # pred 길이만큼 new omission label 리스트 만듦. 
        new_oracle_labels = [[] for _ in range(len(preds))] # pred 길이만큼 new oracle label 리스트 만듦. 

        for i in range(len(sents)):  # 문장 개수만큼 돕니다
            for j, p in enumerate(preds): 
                # print("i: ", i)
                # print("j:", j,"p: ", p)
                # print("p[source]: ",p['source'])
                # print("oracles: ", oracles)
                # print("labels: ", labels)
                # print("ref: ", references) 
                if args.domain != 'all' and args.domain != p['source']:  #args.domain이 all이거나 p['source']가 아니면 
                    continue
                if i in oracles: # i 번째 문장에 있는 단어가 oracles 에 있다면 
                    if i in labels[j]: # i 번쨰 단어가 j번째 dict의 labels
                        la = 2
                    else:
                        la = 1 # 없다면 1 
                else:
